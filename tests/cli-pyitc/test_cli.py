...
TEMP_DIR = "/tmp/pyitc-testing/test-cli"
...
def test_is_valid():
    bad_file_path = os.path.realpath( f"{TEMP_DIR}/test_is_valid/bad_itc" )
    ...
    assert os.path.exists( BINARY_FILE_PATH ), "intelchain cli did not download"
...
def test_bad_bin_set():
    bad_file_path = os.path.realpath( f"{TEMP_DIR}/test_bad_bin_set/itc" )
    shutil.rmtree( Path( bad_file_path ).parent, ignore_errors = True )
    os.makedirs( Path( bad_file_path ).parent, exist_ok = True )
    Path( bad_file_path ).touch()
...
def test_update_keystore():
    ...
    raw_cli_keys_list_print = cli.single_call( "itc keys list", timeout = 2 )
