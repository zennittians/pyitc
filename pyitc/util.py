"""
Basic pyitc utils like is_shard_active
...
def get_bls_build_variables():
    ...
    itc_path = f"{get_gopath()}/src/github.com/zennittians"
    bls_dir = f"{itc_path}/bls"
    mcl_dir = f"{itc_path}/mcl"
    assert os.path.exists( bls_dir ), f"intelchain BLS repo not found at {bls_dir}"
    assert os.path.exists( mcl_dir ), f"intelchain MCL repo not found at {mcl_dir}"
