...
from .util import chain_id_to_int, convert_itc_to_hex

intelchain_FORMATTERS = dict(
    ETHEREUM_FORMATTERS,
    shardID=hexstr_if_str(to_int),  # additional fields forIntelchaintransaction
    toShardID=hexstr_if_str(to_int),  # which may be cross shard
)


class UnsignedintelchainTxData( HashableRLP ):
    """
    UnsignedIntelchaintransaction data
...
class SignedintelchainTxData( HashableRLP ):
    """
    SignedIntelchaintransaction data
    Includes `shardID` and `toShardID`
    as the difference against Eth
    """

    fields = UnsignedintelchainTxData._meta.fields + (
...
def encode_transaction( unsigned_transaction, vrs ):
    ...
    if isinstance(
        ...
        ( UnsignedintelchainTxData,
          SignedintelchainTxData )
    ):
        serializer = SignedintelchainTxData
    else:
...
def serialize_transaction( filled_transaction ):
    ...
    if "v" in filled_transaction:
        # https://github.com/zennittians/intelchain/blob/f8879f5e0288157bf95ae2898a9a27f0c85ff9ad/core/types/transaction_signing.go#L173
        ...
        if "shardID" in filled_transaction:
            serializer = UnsignedintelchainTxData
        else:
...
def sanitize_transaction( transaction_dict, private_key ):
    ...
    if "from" in sanitized_transaction:
        sanitized_transaction[ "from" ] = convert_itc_to_hex(
...
def sign_transaction( transaction_dict, private_key ) -> SignedTransaction:
    ...
    transaction_dict: :obj:`dict` with the following keys
        ...
        gasPrice: :obj:`int` Transaction gas price in INTELLI
        gas: :obj:`int` Gas limit in INTELLI
        to: :obj:`str` Destination address
        value: :obj:`int` Amount to be transferred in INTELLI
    ...
    if "to" in sanitized_transaction and sanitized_transaction[ "to"
        ...
        sanitized_transaction[ "to" ] = convert_itc_to_hex(
    ...
    filled_transaction = pipe(
        ...
        partial( merge,
                 TRANSACTION_DEFAULTS ),
        chain_id_to_v,
        apply_formatters_to_dict( intelchain_FORMATTERS ),
    )
