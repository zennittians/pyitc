"""Wrapper for Intelchain's CLI.

This module makes it easy for one to interact with the Intelchain CLI.
...
Example:
    Below is a demo of how to import, manage keys, and interact with the CLI::
        >>> from pyitc import cli
        >>> cli.single_call("itc keys add test1")
        ...
        ['test1']
        >>> cli.single_call("itc keys list", timeout=2)
        'NAME \t\t ADDRESS\n\ntest1 \tone1aqfeed538xf7n0cfh60tjaeat7yw333pmj6sfu\n'
        >>> cli.get_accounts_keystore()
        {}
...
Example:
    Below is a demo of how to set the CLI binary used by the module::
        ...
        >>> new_path
        '/Users/danielvdm/go/src/github.com/zennittians/pyitc/bin/test'
        >>> from pyitc import cli
...
ARG_PREFIX = "__PYITC_ARG_PREFIX__"
...
def account_keystore_path( value = None ):
    ...
    if "value" not in account_keystore_path.__dict__:
        account_keystore_path.value = "~/.itc/account-keys"
    if value:
...
def _get_current_accounts_keystore():
    ...
    """
    curr_addresses = {}
    response = single_call( "itc keys list" )
...
def _set_account_keystore_path():
    ...
    binary."""
    response = single_call( "itc keys location" ).strip()
...
def _make_call_command( command ):
    ...
    if re.match( ".*itc", command_toks[ 0 ] ):
...
def is_valid_binary( path ):
    """
    :param path: Path to theIntelchainCLI binary (absolute or relative).
    ...
    try:
        with subprocess.Popen(
            ...
            if not err:
                return False
            return "intelchain" in err.decode().strip().lower()
    except (
        OSError,
...
def single_call( command, timeout = 60, error_ok = False ):
    ...
    :returns: Decoded string of response from itc CLI call
...
def download( path = "./bin/itc", replace = True, verbose = True ):
    ...
    if not os.path.exists( path ) or replace:
        ...
        os.chdir( parent_dir )
        itc_script_path = os.path.join( parent_dir, "itc.sh" )
        with open( itc_script_path, "w", encoding = 'utf8' ) as script_file:
            script_file.write(
                requests.get(
                    "https://raw.githubusercontent.com/zennittians/go-sdk/master/scripts/itc.sh"
                ).content.decode()
            )
        os.chmod(
            itc_script_path,
            os.stat( itc_script_path ).st_mode | stat.S_IEXEC
        )
        ...
        if verbose:
            subprocess.call( [ itc_script_path, "-d" ] )
        else:
        ...
        os.rename( os.path.join( parent_dir, "itc" ), path )
        if same_name_file:
            os.rename(
                os.path.join( parent_dir,
                              ".itc_tmp" ),
        ...
        if verbose:
            print( f"SavedIntelchainbinary to: `{path}`" )
        os.chdir( old_cwd )
    ...
    if sys.platform.startswith( "darwin" ):  # Dynamic linking for darwin
        try:
            ...
            if files_in_parent_dir.intersection( _libs ) == _libs:
                ...
                f"{get_gopath()}/src/github.com/zennittians/bls"
            ) and os.path.exists(
                f"{get_gopath()}/src/github.com/zennittians/mcl"
            ):
                env.update( get_bls_build_variables() )
            else:
                raise RuntimeWarning(
                    f"Could not get environment for downloaded itc CLI at `{path}`"
                )
