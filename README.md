# pyitc - Intelchain's python utilities

**This library only supports Python 3.6+**

A Python library for interacting and working the [Intelchain blockchain](https://intelchain.org/)
and [related codebases](https://github.com/zennittians).

[Full documentation is located on Intelchain's GitBook](https://docs.intelchain.org/) (in progress).
...
```
On MacOS:
Make sure you have Python3 installed, and use python3 to install pyitc
```bash
sudo pip3 install pathlib
sudo pip3 install pyitc
```
...
## Running tests
Before you can run tests, you need the python dependencies (`make install`), `docker` and `go` installed to quickly run a local blockchain with staking enabled (detailed instructions [here](https://github.com/zennittians/intelchain/blob/main/README.md)):
```bash
...
## Usage
```py
test_net = 'https://api.s0.b.intelchain.org'				# this is shard 0
test_net_shard_1 = 'https://api.s1.b.intelchain.org'
test_address = 'one18t4yj4fuutj83uwqckkvxp9gfa0568uc48ggj7'

main_net = 'https://rpc.s0.t.intelchain.org'
main_net_shard_1 = 'https://rpc.s1.t.intelchain.org'
```
#### utilities
##### Address conversion
```py
from pyitc import util
hex_addr = util.convert_itc_to_hex('one155jp2y76nazx8uw5sa94fr0m4s5aj8e5xm6fu3')
one_addr = util.convert_hex_to_itc('0xA5241513DA9F4463F1d4874b548dFBAC29D91f34')
```
##### Ether / Wei conversion
```py
from pyitc import numbers
one_ether_in_wei = numbers.convert_itc_to_intelli(1) # as a decimal.Decimal
wei_to_ether = numbers.convert_intelli_to_itc(int(1e18))
```
#### accounts
```py
from pyitc import account
```
##### Balance / account related information
````py
balance = account.get_balance(test_address, endpoint=test_net)				# on shard 0, in INTELLI
total_balance = account.get_total_balance(test_address, endpoint=test_net)	# on all shards, in INTELLI
...
```py
from pyitc.validator import Validator
...
```py
from pyitc.numbers import convert_itc_to_intelli
...
info = {
        ...
        'min-self-delegation': convert_itc_to_intelli(10000),
        'amount': convert_itc_to_intelli(10001),
        'max-rate': '0.9',
        ...
        'max-total-delegation': convert_itc_to_intelli(40000)
    }
validator.load(info)
```
...
```py
from pyitc import staking_structures, staking_signinge
tx = {
  'chainId': 2,
...
### Contracts
```py
from pyitc import contract
from pyitc.util import convert_itc_to_hex
contract_addr = 'one1rcs4yy4kln53ux60qdeuhhvpygn2sutn500dhw'
```
Call a contract without saving state
```py
from pyitc import numbers
result = contract.call(convert_itc_to_hex(contract_addr), 'latest', value=hex(int(numbers.convert_itc_to_intelli(5)))
, gas_price=hex(1), gas=hex(100000), endpoint=test_net)
```
Estimate gas required for a smart contract call
```py
estimated_gas = contract.estimate_gas(convert_itc_to_hex(contract_addr), endpoint=test_net)
```
Fetch the byte code of the contract
```py
byte_code = contract.get_code(convert_itc_to_hex(contract_addr), 'latest', endpoint=test_net)
```
Get storage in the contract at `key`
```py
storage = contract.get_storage_at(convert_itc_to_hex(contract_addr), key='0x0', block_num='latest', endpoint=test_net)
```
...
lottery = w3.eth.contract(abi=contract_abi, address=convert_itc_to_hex('one1rcs4yy4kln53ux60qdeuhhvpygn2sutn500dhw'))
...
```py
from pyitc import staking_structures, staking_signing
```
